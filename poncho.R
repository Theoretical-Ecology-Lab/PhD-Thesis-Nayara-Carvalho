...
file=NULL,title=NULL,xlab="Ordered places", hadj=1,
...
axis(1, at=((0:(nrow(tabela)-1))*dimx+(1:nrow(tabela))*dimx-dimx*.1)/2,
     labels=rownames(tabela),
     las=2, hadj = hadj, font = 3, col.axis = "gray30",
     cex.axis=if(is.null(cex.species)){
       ifelse(30/ncol(tabela)>1,1,30/ncol(tabela))
       }else{cex.species}, tick=F)
}
